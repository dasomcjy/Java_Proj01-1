...
class Bb implements B {
	...
	//@Override 
	//void abc() {		//부모의 인터페이스를 생략되어 있는 것을 적어줘야 오류발생이 없다.
...
public class Interface01 {
		...
		/*
		   인터페이스 (Interface) : 내부와 외부를 연결시켜주는 접점, 콘센트 
		   		...
		   		- 모든 필드는 public static final이 생략되어 있음
		   			 : 값을 수정할 수 없다.
		   		- 모든 메소드는 public abstract이 생략됨 (예외, default , static 메소드 : 1.8 버전)
		   		     : 모든 메소드는 선언부만 있어야 한다. 구현부가 있으면 안됨 : {} <있으면 안됨
		   		     : 예외, default, static 메소드는 인터페이스에서 유일하게 구현부를 가지는 메소드
   		     	- 하나의 클래스는 하나의 부모클래스를 상속해야 한다. , 자바는 다중 상속 불가.
	...
	//	C c1 = new C();		//C : 추상 클래스
		...
		C c2 = new Cc(); 
		System.out.println(c2.m);	//인스턴스 필드 : 객체 생성후에 호출이 가능 	
		//System.out.println(C.m);	//인스턴스 필드 : 
		
		c2.m = 44;
		System.out.println(c2.m); 		//44 :  값수정이 가능하다.
