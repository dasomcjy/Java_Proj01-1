...
interface A {
	...
	//기존의 와이파이 기능의 메소드만 구현된 상태에서 새로운 기능인 블루투스기능이 추가됨
	//2022 년에 새로운 기능 추가. 
	//default 메소드 : 인터페이스 내에서 구현부가 있는 메소드,
		//모든 자식 클래스에서 상속되어 내려감. 새로운 기능을 추가할때 자식 클래스에서 오류없이 기능추가
		
	default void cde () {			//public 생략됨
...
class B implements A {	//extends Object 가 생략되어 있다.
	...
	public void def() {		// 자식클래스에서 인터페이스의 메소드 호출 : A.super.메소드명
		// 인터페이스의 메소드를 호출시 : 
...
public class interface_Super_Use {
	...
	public static void main(String[] args) {
		...
		//a1을 B 타입으로 다운케스팅해야 def() 메소드를 호출할 수 있다.
		System.out.println("=============================");
		B b1 = (B) a1;
		b1.def();
	}
