...
class A {  //Outer class : 필드, 메소드에 접근 제어자가  붙어 있고,
				//Inner class를 생성하면 Outer class 접근 제어자 상관 없이 접근
	...
	private int d = 6;			// 다른패키지, 같은 패키지에서도 접근불가능, 같은 클래스에서만 접근 
	...
	//이너 클래스 : 인스턴스 이너클래스, 1. 아우터 클래스의 접근제어자와 상관없이 접근, 
			//2. 기능 구현을 외부의 객체를 생성없이 내부에서 바로 처리
	//class B는 내부에 outer class의 필드와 메소드를 활용해서 기능 구현을 함.
	class B {
		void bcd() {
